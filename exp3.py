...
for i_episode in range(5000):
	...
	while not done:
		...
		if leaf == qtree.left:
			left_leaf_dataset.append((state, delta_q))
...
fig, axs = plt.subplots(2, 2)
states_distribution = [s for (s, q) in left_leaf_dataset]
q_distribution = [q for (s, q) in left_leaf_dataset]
axs[0, 0].scatter([a0 for (a0, a1, a2, a3) in states_distribution], q_distribution, s=2, color="green")
axs[0, 0].set_xlabel("Cart Position")
axs[0, 0].set_ylabel("$\Delta Q(s_{PA \leq 0}; LEFT)$")
axs[1, 0].scatter([a1 for (a0, a1, a2, a3) in states_distribution], q_distribution, s=2, color="blue")
axs[1, 0].set_xlabel("Cart Velocity")
axs[1, 0].set_ylabel("$\Delta Q(s_{PA \leq 0}; LEFT)$")
axs[0, 1].scatter([a2 for (a0, a1, a2, a3) in states_distribution], q_distribution, s=2, color="orange")
axs[0, 1].set_xlabel("Pole Angle")
axs[0, 1].set_ylabel("$\Delta Q(s_{PA \leq 0}; LEFT)$")
axs[1, 1].scatter([a3 for (a0, a1, a2, a3) in states_distribution], q_distribution, s=2, color="purple")
