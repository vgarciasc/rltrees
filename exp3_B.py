...
def mdp_step(state, action, timestep):
	...
	if (state == 2 and next_state == 3) or (state == 3 and next_state == 2):
		reward = 1
	# if (state == 1 and next_state == 0) or (state == 2 and next_state == 3):
	# 	reward = 1
		...
		for cutoff in range(start_value, end_value):
			...
			for action in range(N_ACTIONS):
				...
				if len(L_partition) > 0:
					...
					# print(f"state i <= {cutoff}: action {'LEFT' if action == 0 else 'RIGHT'}, mean = |{'{:.4f}'.format(np.mean(L_partition))}|")
			...
			for action in range(N_ACTIONS):
				...
				if len(R_partition) > 0:
					...
					# print(f"state i > {cutoff}: action {'LEFT' if action == 0 else 'RIGHT'}, mean = |{'{:.4f}'.format(np.mean(R_partition))}|")
...
def grow_tree(tree, leaf):
	split = determine_split(leaf)

	new_node = CTNode(split, None, None)
...
for i_episode in range(5000):
	...
	while not done:
		...
		if np.random.random() < (eps / (episodes_since_growth / 10 + 1)):
		...
		if len(H) > 5000 and len(H) % 500 == 0:
			sigma = np.std(H)
			normalized_sigma = (sigma / leaf.q_values[action])

			if normalized_sigma > 0.05:
