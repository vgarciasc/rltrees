...
def grow_tree(tree, leaf, env, splitting_criterion, split=None):
    ...
    if env["inherit_history_upon_split"]:
        new_node.left.q_history = copy.deepcopy(leaf.q_history)
        new_node.right.q_history = copy.deepcopy(leaf.q_history)
        new_node.left.state_history = copy.deepcopy(leaf.state_history)
        new_node.right.state_history = copy.deepcopy(leaf.state_history)
...
def prune_tree(qtree, node, env):
    tolerance = env["pruning_tolerance"]
    history = []
    ...
    if parent_of_leaves or parent_of_subtree:
        ...
        tolerance_multiplier = tolerance if average_reward > 0 else (2 - tolerance)
        if new_average_reward < tolerance_multiplier * average_reward:
...
def run_pruned_CUT(env, qtree=None):
    qtree = qtree or QLeaf(parent=None, actions=env['actions'])
    history = []
    ...
    for _ in range(env['cycle_length']):
        ...
        qtree = run_episodes(qtree, env, 
            env['qlearning_episodes_after_growing'],
            should_qlearn=True,
...
if __name__ == "__main__":
    config = {
        ...
        "pruning_tolerance": 0.9,

        "cycle_length": 3,
        "nodes_to_grow": 10, 
        "collection_episodes": 100,
        "reward_estimation_episodes": 100,
    ...
    qtree.reset_values()
    run_episodes(qtree, config, 1000, True, False)
    print(f"Average reward: {get_average_reward(qtree, config, episodes=100)}")
